...
def service_list(request, category):
    ...
    paginator = Paginator(
        services,
        getattr(settings, 'CEDA_SERVICES', {}).get('SERVICES_PER_PAGE', 5)
    )
    try:
    ...
    requests = Request.objects  \
        ...
        .filter_active()  \
        .select_related('role')
    return render(request, 'ceda_services/service_list.html', {
...
def my_services(request):
    ...
    return render(request, 'ceda_services/my_services/service_list.html', {
        'categories': categories,
...
def redirect_to_service(service, view_name = 'service_details'):
    ...
    """
    return redirect('ceda_services:{}'.format(view_name),
...
def service_details(request, service):
    ...
    templates = [
        'ceda_services/{}/{}/service_details.html'.format(
            service.category.name,
            service.name
        ),
        'ceda_services/{}/service_details.html'.format(service.category.name),
        'ceda_services/service_details.html',
    ]
...
def role_apply(request, service, role):
    ...
    templates = [
        'ceda_services/{}/{}/{}/role_apply.html'.format(
            service.category.name,
            service.name,
            role.name
        ),
        ...
        'ceda_services/{}/role_apply.html'.format(service.category.name),
        'ceda_services/role_apply.html',
    ]
    return render(request, templates, {
        'role': role,
        'grant': grant,
...
def service_users(request, service):
    ...
    ``ceda_services.view_users_role`` for at least one role in the service.
    ...
    permission = 'ceda_services.view_users_role'
    ...
    paginator = Paginator(
        grants,
        getattr(settings, 'CEDA_SERVICES', {}).get('GRANTS_PER_PAGE', 20)
    )
    try:
        page = paginator.page(request.GET.get('page'))
    except PageNotAnInteger:
    ...
    if '_apply_filters' in request.GET:
        ...
        preserved_filters = set()
    templates = [
        'ceda_services/{}/{}/service_users.html'.format(
            service.category.name,
            service.name
        ),
        'ceda_services/{}/service_users.html'.format(service.category.name),
        'ceda_services/service_users.html',
    ]
    return render(request, templates, {
        'service': service,
        'statuses': tuple(
    ...
    if request.user.has_perm(permission) or \
       request.user.has_perm(permission, service):
        ...
        if not user_roles:
            ...
            return redirect_to_service(service)
    templates = [
        'ceda_services/{}/{}/service_requests.html'.format(
            service.category.name,
            service.name
        ),
        'ceda_services/{}/service_requests.html'.format(
        ...
        'ceda_services/service_requests.html',
    ]
    return render(request, templates, {
        'service': service,
...
def request_decide(request, pk):
    ...
    if not pending.active or pending.state != RequestState.PENDING:
        ...
        return redirect(
            'ceda_services:service_requests',
    ...
    templates = [
        'ceda_services/{}/{}/request_decide.html'.format(
        ...
        'ceda_services/request_decide.html',
    ]
    return render(request, templates, {
...
def service_message(request, service):
    ...
    # We allow the permission to be allocated for all services, per-service or per-role
    permission = 'ceda_services.send_message_role'
    if request.user.has_perm(permission) or \
       request.user.has_perm(permission, service):
        user_roles = list(service.roles.all())
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            EmailMessage(
                ...
                body = render_to_string('ceda_services/email_message.txt', {
                    'sender': request.user,
        ...
        form = MessageForm()
    templates = [
        'ceda_services/{}/{}/service_message.html'.format(
            service.category.name,
            service.name
        ),
        'ceda_services/{}/service_message.html'.format(service.category.name),
        'ceda_services/service_message.html',
    ]
    return render(request, templates, {
        'service': service,
        'form': form,
