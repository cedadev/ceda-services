...
class Category(models.Model):
    ...
    long_name = models.CharField(
        max_length = 250,
        help_text = 'Long name for the category, used for display'
    )
...
class Service(models.Model):
    ...
    name = models.SlugField(
        max_length = 50,
        help_text = 'The name of the service. This is also used in URLs.'
    )
...
class Role(models.Model):
    ...
    def __str__(self):
        return "{} : {} : {}".format(
            self.service.category.long_name,
            self.service.name,
            self.name
        )
...
class RoleObjectPermission(models.Model):
    ...
    def clean(self):
        ...
        if self.content_type and self.object_pk:
            try:
                _ = self.content_type.get_object_for_this_type(
                    pk = self.object_pk
                )
