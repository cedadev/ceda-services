...
class JoinJISCMailListBehaviour(Behaviour):
    ...
    def apply(self, user):
        ...
        send_mail(
            ...
            settings.CEDA_SERVICES['JISCMAIL_TO_ADDRS'],
            fail_silently = True,
        )
...
class Group(ldap.LDAPModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.gidNumber is None:
            ...
            max_gid = self.__class__.objects.filter(gidNumber__isnull = False)  \
...
for grp in settings.CEDA_SERVICES['LDAP_GROUPS']:
    setattr(
        this_module,
        grp['MODEL_NAME'],
        type(grp['MODEL_NAME'], (Group, ), {
            '__module__' : __name__,
            'Meta' : type('Meta', (Group.Meta, ), {
...
class LdapGroupBehaviour(Behaviour):
    ...
    def clean(self):
        ...
        if self.ldap_model and self.group_name:
            ...
            q = self.__class__.objects.filter(ldap_model = self.ldap_model,
