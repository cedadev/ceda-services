...
for grp in settings.CEDA_SERVICES['LDAP_GROUPS']:
...
class ServiceAdmin(admin.ModelAdmin):
    ...
    def save_related(self, request, form, formsets, change):
        ...
        from django.conf import settings
        default_form = Form.objects.get(pk = settings.CEDA_SERVICES['DEFAULT_METADATA_FORM'])
        if not change:
...
class GrantAdmin(HasMetadataModelAdmin):
    ...
    change_form_template = "admin/ceda_services/grant/change_form.html"
...
class RequestAdmin(HasMetadataModelAdmin):
    ...
    def role_link(self, obj):
        if obj.active and obj.state == RequestState.PENDING:
            url = reverse('admin:ceda_services_request_decide',
                          args = (quote(obj.pk), ),
                          current_app=self.admin_site.name)
        else:
            url = reverse('admin:ceda_services_request_change',
                          args = (quote(obj.pk), ),
                          current_app=self.admin_site.name)
    ...
    def get_urls(self):
        return [
            url(
                ...
                name = 'ceda_services_request_decide'
            ),
        ] + super().get_urls()
    ...
    def decide_request(self, request, pk):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect(
                    '{}:ceda_services_request_changelist'.format(
        ...
        return render(
            request,
            'admin/ceda_services/request/decide.html',
            context
        )
